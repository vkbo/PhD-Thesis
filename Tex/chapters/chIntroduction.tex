...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BPI:Kat87} The radial function $R(r)$ for a range of uniform beams cut off at a radius $a$ up to twice the plasma skin depth $k_{pe}^{-1}$. The plot is reproduced from Katsouleas \textit{et al.} \cite{katsouleas:1987}.}
\end{figure}
...
Unlike in the linear case, there is no full theory describing the non-linear case. However, a non-linear kinetic theory has been developed by Lu \textit{et al.} that is valid under certain assumed conditions \cite{lu:2006a,lu:2006}.
...
Figure \ref{Fig:BPI:Match} shows the evolution of the transverse RMS size of an electron beam as it travels through a plasma. The beam has a normalised emittance of $2\unit{\mu m}$, and is initially matched to a plasma density of $7\nexp{14}\unit{cm}^{-3}$. The tail of the beam, in red, is in the plasma bubble and thus sees an ion density equal to the plasma density. The head of the beam, in blue, where the bubble is forming, sees a lower density. It is thus not matched, and instead oscillates around an equilibrium point at a larger radius. The black line is the predicted $\sigma_{r}$ given by Eq. \ref{EQ:BMatch} at initial plasma density, and beam energy as a function of $z$ as the beam is accelerated \cite{berglyd_olsen:2018}.
...
There are several ways to improve the acceleration gradient in a single plasma stage.



As mentioned, high transformer ratios can in be achieved with multiple drive bunches \cite{jing:2006} with a $\lambda_{pe}$ separation as these will resonantly drive increasing wakefields. As this implies higher accelerating gradients, this is desirable. 
