...
Following Ruth \textit{et al.} \cite{ruth:1985}, let us consider a drive bunch of zero length, with $N_{d}$ particles of individual energy $E_{d}$, and $W_{e}(\xi)$ is the wakefield per unit charge. The energy loss of the bunch in the decelerating field is given by:
\begin{equation}
...
\begin{figure}[hbt]
    ...
    \caption{\label{Fig:BPI:Kat87} The radial function $R(r)$ for a range of uniform beams cut of at a radius $a$ up to twice the plasma skin depth $k_{pe}^{-1}$. The plot is reproduced from Katsouleas \textit{et al.} \cite{katsouleas:1987}.}
\end{figure}
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=1.0\linewidth,trim={0mm 0mm 0mm 0mm},clip]{figures/BeamMatching}
...
Figure \ref{Fig:BPI:Match} shows the evolution of the transverse RMS size of an electron beam as it travels through a plasma. The beam has an emittance of $2\unit{\mu m}$, and is initially matched to a plasma density of $7\nexp{14}\unit{cm}^{-3}$. The tail of the beam, in red, is in the plasma bubble and thus sees an ion density equal to the plasma density. The head of the beam, in blue, where the bubble is forming, sees a lower density. The head of the beam is thus not matched, and instead oscillates around an equilibrium point at a larger radius. The black line shows the line given by Eq. \ref{EQ:BMatch} at initial plasma density and beam energy as a function of $z$ as the beam is accelerated \cite{berglyd_olsen:2018}.
...
One experiment, at the Stanford Linear Accelerator Center (SLAC) \cite{blumenfeld:2007}, achieved up to a doubling of energy of electrons with a $42\unit{GeV}$ driver with $1.8\nexp{10}$ particles, corresponding to a total beam energy of $120\unit{J}$. The propagation length of such a beam is on the order of a metre, and the experiment used an $85\unit{cm}$ lithium plasma cell to achieve the energy doubling with a gradient of $52\unit{GeV}$.

Such short accelerator segments are suitable for multi staged accelerators, but they require multiple high energy electron beams. 

% ================================================================================================ %
\subsection{Protons vs. Electrons as Drive Beam}
\label{Int:DBeam:PDPWFA}

For a beam to efficiently drive wakefields, it should have a length $k_{pe}\sigma_{z} \simeq \sqrt{2}$ \cite{lu:2005,lee:2000}, and transverse size $k_{pe}\sigma_{r} \lesssim 1$. Wider beams will cause filamentation instabilities \cite{allen:2012,sentoku:2003}. As mentioned, high transformer ratios can in addition be achieved with multiple drive bunches \cite{jing:2006} with a $\lambda_{pe}$ separation as these will resonantly drive increasing wakefields. 
