...
The first target objective of Run~2 is the successful acceleration of a short electron beam, looking to maximise energy and charge while retaining a low emittance and energy spread.
The results from Run~1 and simulations will dictate the parameters of the Run~2 electron bunch in order to achieve these goals, and to demonstrate that it is indeed possible to produce an accelerated beam with sufficient quality for further experimental applications.

The second target of Run~2 is to demonstrate scalability.
Previous experiments have shown that high accelerating gradients can be achieved in very short plasma stages, but using lasers or electron bunches as drivers, puts constraints on the distances such gradients can be sustained over (see Section~\ref{Int:BPI}).
...
The Rubidium plasma stage currently uses a laser to ionise the vapour, but the availability of high power lasers puts a limit on how long such a plasma channel can be.
There are two main options for Run~2 to get around this challenge.
\begin{enumerate}
    \item Use two stages, where the first stage is used to let the proton bunch undergo self-modulation, and  second plasma stage is used in its entirety for acceleration.
    In this scenario, the electron bunch will be injected between the two stages.
    However, the optics required to steer the electron bunch onto the axis of the proton bunch, a gap between these two stages would be required.
    ...
    This is in particular covered in Section~12 of the 2016 AWAKE Status Report \cite{awake_collaboration:2016}.
    An additional limitation here is that the AWAKE experimental area has limited space for extending beam lines and adding multiple plasma stages \cite{adli:2016a}.
    \item To avoid the problems with staging, the ideal option would be to use a single plasma stage.
    This would require a different approach to the plasma source, [add more stuff here].
\end{enumerate}
